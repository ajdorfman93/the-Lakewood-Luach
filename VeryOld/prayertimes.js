document.addEventListener('DOMContentLoaded', async function () {
  ...
  let currentNusach = 'All';
  let currentTefilah = 'ShowAll';
  ...
  class EntryTime {
    ...
    setDate(date) {
        const [year, month, day] = date.split('-').map1(Number);
  ...
  const conditionMapping = {
    ...
    // Additional codes handled by custom logic:
    '#ERS': null, 
    '#RSE': null,  
    '#UCT': null,  
    '#RET': null,  
  ...
  async function loadAndDisplayPrayerTimes() {
    ...
    if (!dateValue) {
      // If date is not selected yet, do nothing
      return;
    }

    const entryTime = new EntryTime(dateValue);
  ...
  // ---------------------------------------------------------------------
  // #ERS logic (Earliest in Sunday->Friday range)
  // ---------------------------------------------------------------------
  ...
  async function handleRETLogic(records, entryTime) {
    ...
    for (const rec of retRecords) {
      ...
      const [intHh, intMm] = intervalStr.split(':').map1(Number);
  ...
  // ---------------------------------------------------------------------
  // Utility: Convert local Date => "YYYY-MM-DD"
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // Utility: parseTimeOnSameDate: e.g. "10:00 AM" => local Date
  // ---------------------------------------------------------------------
  ...
  function displayRecords(records) {
    ...
    const jsonOutput = records.map1(record => {
        const fields = record.fields;
        return {
  ...
  // ---------------------------------------------------------------------
  // 1) Call loadAndDisplayPrayerTimes() on DOMContentLoaded
  // ---------------------------------------------------------------------
  // If there's already a value in datePicker, it’ll load. Otherwise,
  // user must pick a date to see results.
  loadAndDisplayPrayerTimes();

  // ---------------------------------------------------------------------
  // 2) Also re-load if the user changes the date
  // ---------------------------------------------------------------------
...
// NEW CODE FOR NUSACH
// ---------------------------------------------------------------------
// Event listener for .nusach-button => set currentNusach and do combined filter
document.addEventListener('click', function(e) {
  if (e.target.classList.contains('nusach-button')) {
    currentNusach = e.target.getAttribute('data-nusach') || 'All';
    doCombinedFilter();  // Re-run with new Nusach
  }
});

// Filter function for Nusach
function filterByNusach(records, nusachFilter) {
  if (!nusachFilter || nusachFilter === 'All') {
    return records.slice();
  }
  const nf = nusachFilter.toLowerCase();
  if (nf === 'adus hamizrach') {
    return records.filter(r => r.fields.Nusach && r.fields.Nusach.toLowerCase() === 'adus hamizrach');
  } else if (nf === 'ashkenaz') {
    return records.filter(r => r.fields.Nusach && r.fields.Nusach.toLowerCase() === 'ashkenaz');
  } else if (nf === 'sfard') {
    return records.filter(r => r.fields.Nusach && r.fields.Nusach.toLowerCase() === 'sfard');
  } else if (nf === 'other') {
    return records.filter(r => {
      const nLower = (r.fields.Nusach || '').toLowerCase();
      return !['adus hamizrach','ashkenaz','sfard','unknown'].includes(nLower);
    });
  } else if (nf === 'unknown') {
    return records.filter(r => {
      return (r.fields.Nusach || '').toLowerCase() === 'unknown';
    });
  } else {
    // Fallback => no filter
    return records.slice();
...
// ---------------------------------------------------------------------
// TEFILAH BUTTONS: handle subfilter
// ---------------------------------------------------------------------
document.addEventListener('click', async function(e) {
  if (e.target.classList.contains('tefilah-button')) {
    currentTefilah = e.target.getAttribute('data-tefilah') || 'ShowAll';
    doCombinedFilter();  // Re-run with new Tefilah
  }
});

// Same parseTefilahArray function
...
// Filter function for Tefilah
function filterByTefilah(records, tefilahFilter) {
  if (!tefilahFilter || tefilahFilter === 'ShowAll') {
    return records.slice();
  }
  const tf = tefilahFilter.toLowerCase();
  if (['shachris','mincha','maariv'].includes(tf)) {
    return records.filter(record => {
      const arr = parseTefilahArray(record.fields.Tefilah_Tefilahs);
      return arr.some(t => t.toLowerCase() === tf);
    });
  } else if (tf === 'special') {
    return records.filter(record => {
      const arr = parseTefilahArray(record.fields.Tefilah_Tefilahs);
...
// ---------------------------------------------------------------------
// doCombinedFilter() => apply Nusach first, then Tefilah
// ---------------------------------------------------------------------
function doCombinedFilter() {
  // Start from the day-of-week + holiday base: filteredRecords
  const afterNusach = filterByNusach(filteredRecords, currentNusach);
  const afterTefilah = filterByTefilah(afterNusach, currentTefilah);
  displayRecords(afterTefilah);
}
